...
// ✅ 1. GET COMPANIES (FIXED WITH REVENUE JOIN)
exports.getCompanies = async (req, res) => {
  try {
    const [companies] = await db.query(
  `SELECT 
     c.id, c.name, c.email, c.phone,
     c.industry, c.owner, c.plan, c.employees,
     c.status,
     COALESCE(SUM(r.amount), 0) AS revenue,
     DATE_FORMAT(c.created_at, '%b %Y') AS joinDate,
     CONCAT(
       UPPER(SUBSTRING(TRIM(c.name),1,1)),
       UPPER(SUBSTRING(TRIM(SUBSTRING_INDEX(c.name,' ',-1)),1,1))
     ) AS avatar
   FROM company c
   LEFT JOIN revenue r ON c.id = r.company_id
   GROUP BY c.id
...
// ✅ 2. CREATE COMPANY (REMOVED revenue COLUMN)
exports.createCompany = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO company 
         (name, industry, owner, email, password, phone, plan, employees, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'trial')`,
      [
        name.trim(),
        industry || "Technology",
    ...
    return res.status(201).json({
      ...
      data: {
        ...
        status: "trial",
        revenue: 0, // calculated later dynamically
        avatar,
      },
    });
  } catch (error) {
...
// ✅ 3. TOGGLE SUSPEND (NO CHANGE)
exports.toggleSuspend = async (req, res) => {
  try {
    const { id } = req.params;

    const [[company]] = await db.query(
      "SELECT id, name, status FROM company WHERE id = ?",
      [id]
    );
    ...
    return res.status(200).json({
      status: true,
      message: `Company ${
        newStatus === "suspended" ? "suspended" : "restored"
      } successfully`,
...
// ✅ 4. DELETE COMPANY (ADD REVENUE DELETE)
exports.deleteCompany = async (req, res) => {
  try {
    ...
    // ✅ delete related data
    await db.query("DELETE FROM revenue WHERE company_id = ?", [id]);
